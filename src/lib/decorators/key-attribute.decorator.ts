...
import { DecoFactoryWithClassGet } from ':const/types';
...
export type KeyAttributeDecoratorFactory = { includeModel: boolean } &
  DecoFactoryWithClassGet<AttributeDecorator, (position?: number, label?: string) => KeyAttributeMetadata>;
...
export const KeyAttribute = (keyAttributeName: string, includeModel = true) => {
  ...
  decorator = (
    (position = 0, label) => (target, propertyKey) => {
      if (position === 0) {
      ...
      if (!attrData) throw new Error(`[KeyAttribute] property '${propertyKey}' must be decorated with @Attribute() first`);
